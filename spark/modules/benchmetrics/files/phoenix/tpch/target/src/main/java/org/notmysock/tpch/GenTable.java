...
public class GenTable extends Configured implements Tool {
    ...
    public Path genInput(String table, int scale, int parallel) throws Exception {
        ...
        for(int i = 1; i <= parallel; i++) {
          if(table.equals("all")) {
            out.writeBytes(String.format("$DIR/dbgen/dbgen/dbgen -b $DIR/dbgen/dbgen/dists.dss -f -s %d -C %d -S %d\n", scale, parallel, i));
          } else {
        	out.writeBytes(String.format("$DIR/dbgen/dbgen/dbgen -b $DIR/dbgen/dbgen/dists.dss -f -s %d -C %d -S %d -T %s\n", scale, parallel, i, table));           
          }
        }
        out.close();
        return in;

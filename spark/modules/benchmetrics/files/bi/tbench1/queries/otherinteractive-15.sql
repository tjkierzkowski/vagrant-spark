...
`t1`.`max_calculation_270497503538720781_ok` AS `max_calculation_270497503538720781_ok`,
`t3`.`x_measure__3` AS `min_calculation_270497503524864011_ok`,
...
MAX((CASE WHEN 30 = 0 THEN NULL ELSE CAST((`date_dim_date`.`d_date` - `t0`.`x_measure__0`) AS DOUBLE) / 30 END)) AS `max_calculation_270497503538720781_ok`
...
JOIN (
SELECT MIN(`date_dim_date`.`d_date`) AS `x_measure__0`,
...
JOIN (
SELECT MIN(`t2`.`x_measure__2`) AS `x_measure__3`,
...
`store_sales`.`ss_ticket_number` AS `ss_ticket_number`,
`date_dim_date`.`d_date_sk` AS `d_date_sk`,
MIN(`date_dim_date`.`d_date`) AS `x_measure__2`
FROM `${DB}`.`store_sales` `store_sales`
JOIN `${DB}`.`item` `item` ON (`store_sales`.`ss_item_sk` = `item`.`i_item_sk`)
JOIN `${DB}`.`customer` `customer` ON (`store_sales`.`ss_customer_sk` = `customer`.`c_customer_sk`)
JOIN `${DB}`.`customer_address` `customer_address` ON (`customer`.`c_current_addr_sk` = `customer_address`.`ca_address_sk`)
